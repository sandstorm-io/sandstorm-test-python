...
def index():
    print(grain.PowerboxDescriptor)
    sys.stdout.flush()
...
def offer_cap(cap_id):
    ...
    def offerCap(res):
        ...
        sealedUiViewTag = grain.PowerboxDescriptor.Tag.new_message(id=sealed.SealedUiView.schema.node.id)
        descriptor = grain.PowerboxDescriptor.new_message(tags=[sealedUiViewTag])
